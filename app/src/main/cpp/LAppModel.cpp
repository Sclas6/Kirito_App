...
bool onnippleR = false;
bool onnippleL = false;
    ...
    , _userTimeSeconds(0.0f)
{
    ...
    _idParamTestX = CubismFramework::GetIdManager()->GetId(ParamBustX);
    _idParamTestY = CubismFramework::GetIdManager()->GetId(ParamBustY);
...
void LAppModel::Update()
{
    ...
    //_model->AddParameterValue(_idParamAngleX, _dragX);
    ...
    if(onnippleR==true&&onnippleL==false){
        _model->AddParameterValue(_idParamAngleX, _dragX * 30); // -30から30の値を加える
        _model->AddParameterValue(_idParamAngleY, _dragY * 30);
        _model->AddParameterValue(_idParamAngleZ, _dragX * _dragY * -30);
       // LAppPal::PrintLog("[APP]Now On Right Nipple");
    }
    if(onnippleL==true&&onnippleR==false){
        //LAppPal::PrintLog("[APP]Now On Left Nipple");
    }
...
void LAppModel::TouchNipple(csmFloat32 pointX, csmFloat32 pointY){
    if(fabsf(pointX)<1) {
        if (pointY > -5000) {
            if (pointX < 0) {
                onnippleL = true;
        ...
        LAppPal::PrintLog("[APP]Locating:{x:%.2f y:%.2f}", pointX, pointY);
        LAppPal::PrintLog(reinterpret_cast<const csmChar *>(_idParamAngleX));
    }
    else{
