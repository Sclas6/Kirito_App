...
void LAppLive2DManager::OnTap(csmFloat32 x, csmFloat32 y)
{
    ...
    for (csmUint32 i = 0; i < _models.GetSize(); i++)
    {
        if (_models[i]->HitTest(HitAreaNameHead, x/scale, (y+transY+0.38f)/scale))
        {
            if (DebugLogEnable)
            {
        ...
        else if (_models[i]->HitTest(HitAreaNameBody, x/scale, (y+transY+0.38f)/scale))
        {
            if (DebugLogEnable)
            {
        ...
        if(fabsf(x)>=1&&fabsf(y)>=1) {
            _models[i]->TouchNipple(x, y);
        }
    }
}
void LAppLive2DManager::OnUpdate() const
{
    ...
    //projection.ScaleRelative(1.0f*scale, static_cast<float>(width)*scale / static_cast<float>(height));
    //projection.Scale(1.0f,0.8f);
    projection.Scale(1.0f, static_cast<float>(width) / static_cast<float>(height));
    projection.TranslateY(0.0f);
...
void LAppLive2DManager::ChangeScene(Csm::csmInt32 index)
{
    ...
    ReleaseAllModel();
    //_models.Resize(5000.0f,new LAppModel());
