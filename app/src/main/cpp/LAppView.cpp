    ...
    _renderTarget(SelectTarget_None)
{
    ...
    //_deviceToScreen->Scale(5,50.0f);

    // 画面の表示の拡大縮小や移動の変換を行う行列
    _viewMatrix = new CubismViewMatrix();
    //_viewMatrix->ScaleRelative(5.0f,5.0f);
}
...
void LAppView::Initialize()
{
    ...
    //LAppPal::PrintLog("[APP]Window: {x:%.2f y:%.2f}", right, top);
    ...
    // 表示範囲の設定
    _viewMatrix->SetMaxScale(ViewMaxScale*5); // 限界拡大率
    _viewMatrix->SetMinScale(ViewMinScale); // 限界縮小率

    // 表示できる最大範囲
    _viewMatrix->SetMaxScreenRect(
        ViewLogicalMaxLeft*5,
        ViewLogicalMaxRight*5,
        ViewLogicalMaxBottom*5,
        ViewLogicalMaxTop*5
    );
}
...
void LAppView::InitializeSprite()
{
    ...
    float fHeight = (backgroundTexture->height * 2.2f);
